...
def update_links(send_time):
    ...
    for thread_row in thread_rows:
        ...
        for email_row in email_rows:
            if thread_row['Subject'] == email_row['Subject']:
                has_dealed_row_list.append(email_row['_id'])
                threads_last_time = datetime.strptime(thread_row['Last time'], '%Y-%m-%d %H:%M:%S')
    ...
    for row in need_insert_rows:
        if insert_row_dict.get(row['Subject']) and insert_row_dict[row['Subject']] < row['Date']:
            insert_row_dict[row['Subject']] = row['Date']
        if not insert_row_dict.get(row['Subject']):
            insert_row_dict[row['Subject']] = row['Date']

        # get row_subject_ids_map for update links
        if not row_subject_ids_map.get(row['Subject'], []):
            row_subject_ids_map[row['Subject']] = [row['_id']]
        else:
            row_subject_ids_map[row['Subject']].append(row['_id'])

    new_row_id_list = []
    new_other_rows_ids_map = {}
    if insert_row_dict:
        insert_thread_rows = [{'Subject': row, 'Last time': insert_row_dict[row]} for row in insert_row_dict]
        ...
        for row in new_thread_rows:
            ...
            new_other_rows_ids_map[row['_id']] = row_subject_ids_map[row['Subject']]
